...
class PromiseContext {
    constructor() {
         ...
         * @return {any}
         */
        this.callCatchCallbackOrPropagate = (error) => {
 ...
 * @template FailType
 */
class TypedPromise extends BaseTypedPromise {
    constructor(action) {
        super(new PromiseContext());
    ...
    then(callback, rejected) {
        ...
        if (rejected)
            this.context.failCallback = rejected;
    }
    resolve(value) {
        // if it was cancelled
        if (this.context.status === 'cancelled') {
